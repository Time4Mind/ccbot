...
目录列表立即使用缓存或浅层文件系统/Git 元数据绘制。一次后台扫描会
同时计算所有已访问目录的嵌套内容新鲜度;依赖和缓存目录会被忽略。
...
长 tool-log 之上。第一次进入已完成会话时仍保留 `✅`;只有第二次进入
才会变为 `☑️`。会话内操作以及切换离开不会增加该计数。如果徽章是
`❓`,切换器点击会画出存好的
