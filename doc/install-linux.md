...
В Settings → *Кнопки опций* включается видимость кнопки
*🖥 Терминал*. По умолчанию она скрыта. При включённой видимости
кнопка появляется под *Опциями* только у активной сессии, когда к её
tmux-окну ещё не подключён клиент. Нажатие открывает терминал только
для текущей сессии; автоматического запуска нет.
...
| *🖥 Терминал* не появляется | Кнопка скрыта в Settings, эмулятор не на PATH или к tmux-окну уже подключён клиент | в Settings → *Кнопки опций* включи кнопку и проверь эмулятор |
