...
По умолчанию выводится один JSON-объект:
...
Поле `command` содержит bearer-подобный одноразовый pairing token. Агент
...
Если в payload уже есть `--node-id` и `--name`, их нельзя переписывать. Если
стабильный id не задан, worker использует нормализованный hostname или создаёт
временный `worker-<random>` id. Процесс должен оставаться запущенным; для
