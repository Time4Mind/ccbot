...
Эта FIFO относится только к пользовательским prompt и намеренно остаётся в
памяти. События уже выполняющихся worker-сессий доставляются иначе: worker
сохраняет упорядоченный outbox на диск до подтверждения leader, а leader
сохраняет применённую позицию потока до отправки подтверждения. Поэтому
неподтверждённое событие повторяется после reconnect или штатного restart, но
повтор уже применённого события не создаёт второе Telegram-обновление. Полный
transcript на leader для этого не копируется; история читается с worker по
запросу. Успешная отправка Telegram и локальная запись её receipt не образуют
распределённую транзакцию, поэтому аварийное завершение ровно между этими двумя
операциями остаётся ограниченным best-effort окном.
...
- не переключает и не изменяет source branch автоматически; multi-node runtime
  поставляется из `main`, а изменения проходят обычный branch/PR/CI flow.
