...
RU: dict[str, str] = {
    ...
    "help.body.alerts": (
        ...
        "*Заполнение контекста.* В карточке Codex строка вида "
        "``─── 6-sol high: N% ───`` показывает модель, effort и долю "
        "заполнения. Если модель неизвестна, вместо неё написано ``context``. "
        "Для Codex используются точные token usage и размер "
