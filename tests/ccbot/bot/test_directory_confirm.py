...
@pytest.mark.asyncio
async def test_create_folder_prompt_has_back_to_current_page(monkeypatch, tmp_path) -> None:
...
async def test_folder_name_creates_and_opens_child(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(_messages_text, "maybe_consume_code", AsyncMock(return_value=False))
